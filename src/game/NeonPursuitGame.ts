...
export class NeonPursuitGame {
  ...
  private async createEngine(): Promise<AbstractEngine> {
    try {
      if (await WebGPUEngine.IsSupportedAsync) {
        const engine = new WebGPUEngine(this.canvas, { antialias: true, adaptToDeviceRatio: true, powerPreference: 'high-performance' });
        await engine.initAsync();
        this.hud.renderer.textContent = 'WEBGPU';
        return engine;
      }
    } catch (error) {
      console.warn('WebGPU initialization failed; using WebGL.', error);
    }
    this.hud.renderer.textContent = 'WEBGL FALLBACK';
    return new Engine(this.canvas, true, { adaptToDeviceRatio: true, powerPreference: 'high-performance', stencil: true });
  }
  ...
  private buildWorld(scene: Scene): void {
    ...
    for (let i = -gridRadius; i <= gridRadius; i += 1) {
      ...
      for (let marker = -10; marker <= 10; marker += 1) {
  ...
  private buildRendering(scene: Scene, camera: UniversalCamera): void {
    const glow = new GlowLayer('neon-glow', scene, { blurKernelSize: 32 });
    glow.intensity = 0.55;
